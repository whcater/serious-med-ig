...
* url ^short = "扩充的资料项目名称"
* url ^definition = "扩充的资料项目之定义连结—一个逻辑名称或URL"
* url ^comment = "此定义可直接指向可计算的或人类可读的扩充资料项目的定义，也可以是其他规范中声明的逻辑URI。此定义**必须（SHALL）** 定义扩充的结构定义的URI。"
* value[x] only Age
* value[x] ^short = "扩充的资料项目的内容值"
* value[x] ^definition = "扩充的资料项目的内容值—必须是一组受限制的data type中的一个（清单请见[Extensibility](http://hl7.org/fhir/2021Mar/extensibility.html)）"